...
public class KafkaService {
    ...
    void init() {
        ...
        this.tracer = openTelemetry.getTracer("transaction-kafka-producer", "1.0.0");
    ...
    /**
     * Sends an existing event JSON (from outbox) without creating a new
     * EventEnvelope — preserves the original occurred_at for idempotent replay.
     */
    public Uni<Void> sendExistingEvent(String topic, String key, String eventJson) {
        if (producer == null) {
            return Uni.createFrom().failure(
                    new IllegalStateException("Kafka producer not initialized"));
        }
        if (eventJson == null) {
            return Uni.createFrom().failure(new IllegalArgumentException("Kafka eventJson cannot be null"));
        }
        JsonObject payload = new JsonObject(eventJson);
